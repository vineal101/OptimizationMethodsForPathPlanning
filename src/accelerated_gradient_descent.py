...
def plan(env, t0=0.005, alpha=1e-4, beta=0.5, max_iter=2000, tol=0.05, d0=1.5, restart=True, max_move=0.15):
    p = env.start.copy()
    p_prev = p.copy()
    path = [p.copy()]
    ...
    for k in range(max_iter):
        ...
        step = collision_check(step, y, direction, env.obstacles, beta=beta)
